...
The answer is changing. Larger context windows, smarter compression, and on-demand subagent spawning make managers unnecessary overhead for most workflows.

## The Orchestration Paradox

We market AI agents as expert-level reasoners, then build systems where:
- **Humans pre-decide** how to split work across agents before seeing the actual problem
- **Static orchestrators** route tasks based on predetermined logic, not dynamic assessment
- **Fixed topologies** assume task structure before the agent evaluates complexity
...
## What Actually Works: Single Agents
...
- **Lead agent** receives intent and decides decomposition dynamically after seeing the problem
- **Spawns subagents** only when genuinely needed for specialized work, via `spawn_subagent()`-style tool calls
- **Manages context** through compression and retrieval, keeping session state coherent
- **Logs centrally** for audit without routing control through a manager

**Benefits:**
- Latency drops (no orchestration hop)
- Complexity drops (fewer services to operate)
- Intelligence increases (decomposition happens with full problem context)
- Flexibility increases (strategy adapts to actual task structure)

## When Managers Still Make Sense

Narrow exceptions exist:
- **Heavy regulatory requirements** demanding centralized compliance engines
- **Multi-tenant routing** with complex quotas, billing, and access control
- **Multi-session human-in-the-loop** workflows across many users over extended time
